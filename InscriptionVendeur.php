...
if ($_POST['button']) {
  $EmailVendeur = ($_POST['EmailVendeur']);
  $PseudoVendeur = ($_POST['PseudoVendeur']);
  $NomVendeur = ($_POST['NomVendeur']);
  $PhotoVendeur = ($_POST['PhotoVendeur']);
  $ImageVendeur = ($_POST['ImageVendeur']);

  if (!empty($EmailVendeur) AND !empty($PseudoVendeur) AND !empty($NomVendeur)) {
    $reqvend = $bdd->prepare('SELECT * FROM vendeur where EmailVendeur = ? ');
    $reqvend->execute(array($EmailVendeur));

    $VendExist = $reqvend->rowCount();

    if ($VendExist == 0) {
        $reqaddvend = $bdd->prepare("INSERT INTO vendeur VALUES( NULL, '$PseudoVendeur', '$EmailVendeur', '$NomVendeur', '$PhotoVendeur', '$ImageVendeur')");
        $reqaddvend->execute();
        $reqaddvend->closeCursor();
                ...
                <div class="col-lg-6">
                  ...
                  <div class="card shadow mb-4">
                    <div class="card-header py-3">
                      <h6 class="m-0 font-weight-bold text-primary">Inscription Vendeur</h6>
                  </div>
                  <div class="card-body">
                    <form action="InscriptionVendeur.php" method="post">
                        <table>
                            ...
                            <tr>
                                <td>Email :</td>
                                <td><input type="text" class="form-control" name="EmailVendeur"></td>
                            </tr>

                            <tr>
                                <td>Pseudo :</td>
                                <td><input type="password" class="form-control" name="PseudoVendeur"></td>
                            </tr>
                            
                            <tr>
                                <td>Nom :</td>
                                <td><input type="text" class="form-control" name="NomVendeur"></td>
                            </tr>

                            <tr>
                                <td>Photo :</td>
                                <td><input type="file"  name="PhotoVendeur" accept="image/png, jpeg"></td>
                            </tr>
                            
                            <tr>
                                <td>Image :</td>
                                <td><input type="text" class="form-control" name="ImageVendeur"></td>
                            </tr>

                            <tr>
                                <td colspan="2" align="center">
                                    <input type="submit" class="btn btn-primary" name="button" value="Inscription">

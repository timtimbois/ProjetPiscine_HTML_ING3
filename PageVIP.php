...
<head>
	...
	<title>EbayECE : Accessoire VIP</title>
					...
					<div class="col-lg-6">
						...
						<div class="card shadow mb-4">
							<div class="card-header py-3">
								<h6 class="m-0 font-weight-bold text-primary">Voici les accessoires VIP <?php echo $_SESSION['PrenomAcheteur']; ?> </h6>
							</div>
							<div class="card-body">
						...
						//Affichage des objets AcheterMaintenant
								...
								$reqselcitem = $bdd->prepare('SELECT IDItem, NomItem, Photo1Item, Photo2Item, VideoItem, DescriptionItem, PrixItem, Photo3Item, CategorieItem, TypeVenteItem FROM item WHERE CategorieItem LIKE "Accessoire VIP" AND TypeVenteItem LIKE "AcheterMaintenant"');
								...
								<div id="GroupeAch"> 
									...
									while ($data = $reqselcitem->fetch()) {
										echo '<li><p id="nom">'.$data['NomItem'].'&emsp;<img src="'.'<img src="'.$data['Photo1Item'].'" height="100" width="120">'.'&emsp;'.'<img src="'.$data['Photo2Item'].'" height="100" width="120">'.'&emsp;'.'<video src="'.$data['VideoItem'].'" height="100" width="120"></video>'.'</p>'.'<p id="DescItem">'.$data['DescriptionItem'].'&emsp;'.$data['PrixItem'].'€</p>'.'<p id="Photo3Item">'.'<img src="'.$data['Photo3Item'].'" height="100" width="120">'.'</p>'.'<p id="CategItem">'.$data['CategorieItem'].'&emsp;'.$data['TypeVenteItem'].'<p> Mis en vente par :'.$data['NomVendeur'].'</p>'.'</li>';
										?>
										
										<?php
										
										echo '<form method="post" action="PageVIP.php">'.
										...
										'<input type="hidden" name="TypeVenteItem" value='.$data['TypeVenteItem'].'>'.
							...
							//MeilleureOffre
									...
									$reqselcitem = $bdd->prepare('SELECT IDItem, NomItem, Photo1Item, Photo2Item, VideoItem, DescriptionItem, PrixItem, Photo3Item, CategorieItem, TypeVenteItem FROM item WHERE CategorieItem LIKE "Accessoire VIP" AND TypeVenteItem LIKE "MeilleureOffre"');
									...
									while ($data = $reqselcitem->fetch()) {
										...
										echo '<form method="post" action="ChatOffre.php">'.
										'<input type="hidden" name="IDItem" value='.$data['IDItem'].'>'.
										'<input type="hidden" name="PrixItem" value='.$data['PrixItem'].'>'.
										'<input type="hidden" name="NomItem" value='.$data['NomItem'].'>'.
										'<input type="hidden" name="Photo1Item" value='.$data['Photo1Item'].'>'.
										'<input type="hidden" name="TypeVenteItem" value='.$data['TypeVenteItem'].'>'.
										'<input type="submit" value="FAIRE UNE OFFRE" class="btn btn-ajout tex">'.
										'</form>';
									...
									$reqselcitem = $bdd->prepare('SELECT IDItem, NomItem, Photo1Item, Photo2Item, VideoItem, DescriptionItem, PrixItem, Photo3Item, CategorieItem, TypeVenteItem FROM item WHERE CategorieItem LIKE "Accessoire VIP" AND TypeVenteItem LIKE "Enchère"');
									...
									while ($data = $reqselcitem->fetch()) {
										...
										'<input type="hidden" name="Photo1Item" value='.$data['Photo1Item'].'>'.
										'<input type="hidden" name="TypeVenteItem" value='.$data['TypeVenteItem'].'>'.
										'<input type="submit" value="ENCHERIR" class="btn btn-ajout tex" onClick="history.go(0)">'.
										'</form>';
									...
									$TypeVenteItem = isset($_POST["TypeVenteItem"])? $_POST["TypeVenteItem"] : "";
									$reqselcitem->closeCursor();
									
									if ($TypeVenteItem=="AcheterMaintenant") {
									...
									if ($TypeVenteItem=="MeilleureOffre"){

									}

									if ($TypeVenteItem=="Enchère"){

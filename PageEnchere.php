					...
					<div class="col-lg-6">
						...
						<div class="card shadow mb-4">
							<div class="card-header py-3">
								<h6 class="m-0 font-weight-bold text-primary">Voici les enchères <?php echo $_SESSION['PrenomAcheteur']; ?> </h6>
							</div>
							<div class="card-body">
								...
								$reqselcitem = $bdd->prepare('SELECT IDItem, NomItem, Photo1Item, Photo2Item, VideoItem, DescriptionItem, PrixItem, Photo3Item, CategorieItem, TypeVenteItem FROM item WHERE TypeVenteItem LIKE "Enchères"');
								...
								<div id="GroupeAch"> 
									...
									while ($data = $reqselcitem->fetch()) {
										echo '<li><p id="nom">'.$data['NomItem'].'&emsp;'.$data['Photo1Item'].'&emsp;'.$data['Photo2Item'].'&emsp;'.$data['VideoItem'].'</p>'.'<p id="DescItem">'.$data['DescriptionItem'].'&emsp;'.$data['PrixItem'].'€</p>'.'<p id="Photo3Item">'.$data['Photo3Item'].'</p>'.'<p id="CategItem">'.$data['CategorieItem'].'&emsp;'.$data['TypeVenteItem'].'<p> Mis en vente par :'.$data['NomVendeur'].'</p>'.'</li>';



										echo '<form method="post" action="PageEnchere.php">'.
